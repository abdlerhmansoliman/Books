    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-100">
        <div class="w-full  bg-white rounded-2xl shadow-md p-8 ">
            ...
            <form method="POST" action="{{  route('dashboard.user.update', $user)  }}" class="space-y-4" enctype="multipart/form-data">
                ...
                <!-- roles-->
                <div>
                    <label class="block text-sm text-gray-600 mb-1">{{__('messages.roles')}}</label>
                    <select name="roles[]">
                ...
                <div>
                    <label class="block text-sm text-gray-600 mb-1">{{__('messages.status')}} </label>
                    <select name="statuses[]">
                        @foreach($statueses as $statues)
                        <option value="{{ $statues->id }}" 
                            @if($user->statuse->contains($statues->id)) selected @endif>
                            {{ $statues->name }}
                        </option>
                    @endforeach
                    </select>
