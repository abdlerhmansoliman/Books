<x-dash-layout>
    <div class="bg-white mt-4 border border-blue-200 rounded-md p-6 max-w-4xl mx-auto ">
        ...
        <form action="{{ route('book.update',$book) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            <!-- الحقول الرئيسية -->
          @csrf  
          @method('PATCH')  
                  <!-- الحقول الرئيسية -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="form-group">
                    ...
                    <input type="text" value="{{ old('title', $book->title) }}" name="title" class="w-full border border-gray-300 rounded py-2 px-3 text-gray-500 bg-gray-50" placeholder="{{ __('messages.book name') }}" />
                </div>
  
                <!-- اسم المؤلف -->
                <div class="form-group">
                    <label class="block text-sm font-medium text-blue-900 mb-1">{{ __('messages.author name') }} *</label>
                    <input type="text" name="auth_name" value="{{ old('auth_name', $book->auth_name) }}" class="w-full border border-gray-300 rounded py-2 px-3 text-gray-500 bg-gray-50" placeholder="{{ __('messages.author name') }}" />
                </div>
              ...
              <div class="form-group">
                  ...
                  <input type="number" name="pages" value="{{ old('pages', $book->pages) }}" class="w-full border border-gray-300 rounded py-2 px-3 text-gray-500 bg-gray-50" placeholder="{{ __('messages.pages') }}" />
              </div>
  
                <!-- الوصف -->
                <div class="form-group md:col-span-2">
                    <label class="block text-sm font-medium text-blue-900 mb-1">{{ __('messages.book description') }} *</label>
                    <textarea rows="5" name="description" class="w-full border border-gray-300 rounded py-2 px-3 text-gray-500 bg-gray-50" placeholder="{{ __('messages.book description') }}">
                ...
                <div class=" form-group">
                  ...
                  <input type="number" value="{{ old('publication_year', $book->publication_year) }}" name="publication_year" class="w-full border border-gray-300 rounded py-2 px-3 text-gray-500 bg-gray-50" placeholder="{{__('messages.publish date')}} " />
              </div>
                <!-- رفع الملف -->
                <div class="form-group">
                ...
                <div class=" form-group">
                  ...
                  <input type="text" name="language" value="{{ old('language', $book->language) }}" class="w-full border border-gray-300 rounded py-2 px-3 text-gray-500 bg-gray-50" placeholder="{{__('messages.blanguage')}} " />
